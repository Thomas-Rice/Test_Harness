...
class cameraSolverTemplate():
	def __init__(self, scriptName):
		self.scriptName = scriptName
		# Create CameraSolver and Read nodes
		self.CameraSolverInstance = BCTC.cameraSolverOps(scriptName)

	def returnNode(self):
		# Do Specific Test Setup and get variables
		self.node, self.filePath = self.CameraSolverInstance.setup()
		return self.node

	def writeOut(self):
		''' WRITE OUT '''
		#Get the path to the images so we can compare
		pathToCompare = self.CameraSolverInstance.writeOut(self.node)
